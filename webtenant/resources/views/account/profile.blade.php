...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#frmchangepass").validate({
            ...
            messages: {
                // password1: {confirmpass: "Password is not valid"},
        ...
        $('#btnSave').click(function(){
            // console.log($('#frmEditor'));
            // console.log($('#frmEditor').valid());return;
            ...
            obj.isFile = isFile;
                if(isFile){
                    ...
                    //     }
                    // });
                    var site_url = "{{ url('account/updateprofile') }}";
                    $.ajax({
                        url: site_url,
                        type: "POST",
                        data: dataform,
        ...
        $('#btnSavepass').click(function(){
            // console.log($('#frmchangepass').valid());return;
            if($('#frmchangepass').valid()){
                ...
                // console.log(email)
                dataform.push({name:"email",value:email},{name:"password",value:password},{name:"_token",value:"{{ csrf_token() }}"})
                var site_url = "{{ url('account/changepass') }}";
                $.ajax({
                    url: site_url,
                    type: "POST",
                    data: dataform,
    ...
    function loaddata(){
        ...
        if (Id.length > 0) {
            $.getJSON("{{ url('account/getbyemail') }}" + "/" + Id, function (data) {
